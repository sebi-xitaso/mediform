...
import { createRecord, updateStatus, _clearStore } from "../store.js";
...
describe("POST /staff/questionnaires/:id/review", () => {
  ...
  it("transitions to Approved on approve decision", async () => {
    ...
    const res = await req(
      "POST",
      `/staff/questionnaires/${record.id}/review`,
      { decision: "approve" }
    );
    expect(res.status).toBe(200);
    const body = await res.json() as any;
  ...
  it("transitions to Draft with feedback on request_changes (BR-025)", async () => {
    ...
    const res = await req(
      "POST",
      `/staff/questionnaires/${record.id}/review`,
      { decision: "request_changes", feedback: "Please add LOINC codes." }
    );
    expect(res.status).toBe(200);
    const body = await res.json() as any;
    expect(body.questionnaire.status).toBe("draft");
    expect(body.questionnaire.reviewFeedback).toBe(
      "Please add LOINC codes."
    );
  });
  ...
  it("transitions to Rejected with reason on reject", async () => {
    ...
    const res = await req(
      "POST",
      `/staff/questionnaires/${record.id}/review`,
      { decision: "reject", feedback: "Out of scope for this clinic." }
    );
    expect(res.status).toBe(200);
    const body = await res.json() as any;
  ...
  it("returns 404 when questionnaire does not exist", async () => {
    const res = await req(
      "POST",
      "/staff/questionnaires/no-such-id/review",
      { decision: "approve" }
    );
    expect(res.status).toBe(404);
  });
  ...
  it("returns 409 when questionnaire is not in Review status (BR-021)", async () => {
    ...
    const res = await req(
      "POST",
      `/staff/questionnaires/${r.id}/review`,
      { decision: "approve" }
    );
