...
import type { Answer, ParsedQuestionnaire } from "../types.js";
...
describe("compileResponse – QuestionnaireResponse", () => {
	...
	it("sets the questionnaire reference when fhirQuestionnaireRef is provided", () => {
		...
		expect(result.questionnaireResponse.questionnaire).toBe(
...
describe("compileResponse – Observations", () => {
	...
	it("sets derivedFrom reference to the QuestionnaireResponse", () => {
		...
		for (const obs of result.observations) {
			expect(obs.derivedFrom?.[0].reference).toContain(
				"QuestionnaireResponse/",
			);
		}
	});
	...
	it("maps integer answer to valueInteger", () => {
		...
		const _obs = result.observations.find((_o) =>
		...
		const _intensityObs =
			result.observations[
				result.observations.findIndex(
					(_, idx) =>
						result.questionnaireResponse.item[idx]?.linkId === "pain-intensity",
				)
			];
	...
	it("maps boolean answer to valueBoolean", () => {
		...
		const boolObs = result.observations.find(
			(o) => o.valueBoolean !== undefined,
		);
	...
	it("maps Coding answer to valueCoding", () => {
		...
		const codingObs = result.observations.find(
			(o) => o.valueCoding !== undefined,
		);
