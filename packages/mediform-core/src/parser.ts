...
export function parseMediform(source: string): ParseResult {
  ...
  if (frontmatterText === null) {
    errors.push({ line: 1, message: "Missing frontmatter: file must start with ---" });
...
function parseFrontmatterYaml(
  ...
  if (!kvMap.title || typeof kvMap.title !== "string") {
    errors.push({ line: 1, message: "Frontmatter missing required field: title" });
    hasError = true;
  }
  if (kvMap.version === undefined || kvMap.version === null) {
    errors.push({ line: 1, message: "Frontmatter missing required field: version" });
    hasError = true;
  }
  if (!kvMap.status || typeof kvMap.status !== "string") {
    errors.push({ line: 1, message: "Frontmatter missing required field: status" });
    hasError = true;
...
function parseBody(
  ...
  const finalizeQuestion = () => {
    ...
    const q = buildQuestion(currentQuestion, lineOffset + (currentQuestion.line ?? 0), errors);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("# ")) {
      finalizeQuestion();
      currentQuestion = { title: line.slice(2).trim(), line: i + 1, metadata: {}, optionLines: [], descLines: [] };
      continue;
    }
...
function buildQuestion(
  ...
  if (!rawType) {
    errors.push({ line: lineNum, message: `Question "${raw.title}": missing required field "type"` });
    hasError = true;
  } else if (!VALID_TYPES.has(rawType)) {
    errors.push({ line: lineNum, message: `Question "${raw.title}": unknown type "${rawType}"` });
    hasError = true;
  }
  ...
  if (rawRequired !== undefined) {
    if (rawRequired !== "true" && rawRequired !== "false") {
      errors.push({ line: lineNum, message: `Question "${raw.title}": "required" must be true or false, got "${rawRequired}"` });
      hasError = true;
    } else {
  ...
  if (rawMapsTo && !VALID_MAPS_TO.has(rawMapsTo)) {
    errors.push({ line: lineNum, message: `Question "${raw.title}": unknown maps-to "${rawMapsTo}"` });
    hasError = true;
  }
